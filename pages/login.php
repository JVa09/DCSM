...
// ── ACCESS GATE ──────────────────────────────────────────────────────────────
// Login page is unlisted. Share only the full URL (with ?key=) with admins.
// Change ADMIN_ACCESS_KEY to a hard-to-guess value before deploying.
define('ADMIN_ACCESS_KEY', 'DICT-ADMIN-2026');
...
$client_ip    = trim(explode(',', $_SERVER['HTTP_X_FORWARDED_FOR'] ?? $_SERVER['REMOTE_ADDR'] ?? '127.0.0.1')[0]);
$window_start = date('Y-m-d H:i:s', time() - LOCKOUT_MINS * 60);

// Bootstrap table on first run
...
$chk = $conn->prepare("SELECT COUNT(*) AS cnt, MIN(attempted_at) AS oldest FROM login_attempts WHERE ip_address=? AND attempted_at>?");
$chk->bind_param("ss", $client_ip, $window_start);
$chk->execute();
$rate = $chk->get_result()->fetch_assoc();
$chk->close();

if ((int)$rate['cnt'] >= MAX_ATTEMPTS) {
    $is_locked        = true;
    $lockout_until_ts = strtotime($rate['oldest']) + LOCKOUT_MINS * 60;
    $login_error      = 'lockout';
} else {
    $attempts_remaining = MAX_ATTEMPTS - (int)$rate['cnt'];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$is_locked) {
    ...
    $stmt = $conn->prepare("SELECT id, username, password, role, region, province FROM admin_users WHERE username = ? LIMIT 1");
    ...
    if ($user = $result->fetch_assoc()) {
        if (password_verify($password, $user['password'])) {
            if ($user['role'] === 'superadmin' ||
                (trim($user['region']) === trim($region) && trim($user['province']) === trim($province))) {

                // Clear attempt log for this IP on successful login
                $del = $conn->prepare("DELETE FROM login_attempts WHERE ip_address=?");
                $del->bind_param("s", $client_ip);
                $del->execute();
                $del->close();
                ...
                $location_text = ($user['role'] === 'superadmin') ? "National/System Wide" : $province;
                log_activity($conn, 'Login', 'Authentication', "User {$user['username']} logged in successfully ({$location_text}).");

                header("Location: ../admin/pages/dashboard.php");
                exit();
            } else {
                $login_error = "Access denied for this region/province.";
    ...
    // Record failed attempt then re-evaluate lockout state
    if (!empty($login_error)) {
        $ins = $conn->prepare("INSERT INTO login_attempts (ip_address) VALUES (?)");
        $ins->bind_param("s", $client_ip);
        $ins->execute();
        $ins->close();

        $chk2 = $conn->prepare("SELECT COUNT(*) AS cnt, MIN(attempted_at) AS oldest FROM login_attempts WHERE ip_address=? AND attempted_at>?");
        $chk2->bind_param("ss", $client_ip, $window_start);
        $chk2->execute();
        $rate2 = $chk2->get_result()->fetch_assoc();
        $chk2->close();

        if ((int)$rate2['cnt'] >= MAX_ATTEMPTS) {
            $is_locked        = true;
            $lockout_until_ts = strtotime($rate2['oldest']) + LOCKOUT_MINS * 60;
            $login_error      = 'lockout';
        } else {
            $attempts_remaining = MAX_ATTEMPTS - (int)$rate2['cnt'];
        }
    }
}
?>
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: "Segoe UI", Arial, sans-serif; }
        ...
        .alert-error > div { flex: 1; }
        .alert-error p { margin-top: 3px; font-size: 12px; opacity: .9; }
        .alert-lockout { background: #fffbeb; border-color: #fde68a; border-left-color: #f59e0b; color: #92400e; animation: none; }
        .alert-lockout i { color: #f59e0b; font-size: 18px; }
        .warn-remaining { color: #92400e; font-weight: 600; }
        .btn-login:disabled { opacity: .5; cursor: not-allowed; transform: none !important; box-shadow: none; }
...
<div class="card-wrap">
    ...
    <div class="form-side">
        ...
        <?php if ($login_error === 'lockout'): ?>
        <div class="alert-error alert-lockout">
            <i class="fas fa-clock-rotate-left"></i>
            <div>
                <strong>Too Many Failed Attempts</strong>
                <p>Access locked. Try again in <span id="lockoutTimer">--:--</span>.</p>
            </div>
        </div>
        ...
        <div class="alert-error">
            ...
            <div>
                ...
                <?php if ($attempts_remaining < MAX_ATTEMPTS): ?>
                <p class="warn-remaining"><?= $attempts_remaining ?> attempt<?= $attempts_remaining !== 1 ? 's' : '' ?> remaining before <?= LOCKOUT_MINS ?>-minute lockout.</p>
                <?php endif; ?>
            </div>
        </div>
        <?php endif; ?>
        ...
        <form method="POST" action="">
            ...
            <button type="submit" class="btn-login" <?= $is_locked ? 'disabled' : '' ?>>
...
<?php if ($is_locked && $lockout_until_ts > 0): ?>
(function() {
    var until = <?= $lockout_until_ts ?> * 1000;
    function tick() {
