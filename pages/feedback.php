...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: "Segoe UI", Arial, sans-serif; }
        ...
        .vgate-backdrop.hidden { opacity: 0; visibility: hidden; pointer-events: none; }
        .vgate-card {
        ...
        .vgate-icon {
            ...
            background: linear-gradient(135deg, var(--secondary), var(--primary));
            display: flex; align-items: center; justify-content: center;
            font-size: 32px; box-shadow: 0 8px 24px rgba(0,56,168,.4);
        }
        .vgate-card h2 { font-size: 22px; font-weight: 800; color: var(--primary); margin-bottom: 8px; }
        .vgate-card p  { font-size: 14px; color: var(--muted); margin-bottom: 28px; line-height: 1.6; }
        .vgate-input-wrap { position: relative; margin-bottom: 14px; }
        .vgate-input {
            width: 100%; padding: 15px 18px; border-radius: 14px;
            border: 2px solid var(--border); font-size: 18px; font-weight: 700;
            text-align: center; letter-spacing: 2px; text-transform: uppercase;
            color: var(--primary); background: #f8fafc;
            transition: all .25s ease; font-family: 'Courier New', monospace;
        }
        .vgate-input:focus { outline: none; border-color: var(--secondary); background: #fff; box-shadow: 0 0 0 4px rgba(0,56,168,.1); }
        .vgate-input.err { border-color: var(--danger); background: #fff5f5; animation: shake .4s ease; }
        .vgate-input.ok  { border-color: var(--success); background: #f0fdf4; }
        .vgate-msg { font-size: 13px; font-weight: 600; min-height: 20px; margin-bottom: 0; transition: color .2s; }
        .vgate-msg.err { color: var(--danger); }
        .vgate-msg.ok  { color: var(--success); }
        .vgate-btn {
            margin-top: 16px;
            width: 100%; padding: 15px; border: none; border-radius: 14px;
            background: linear-gradient(135deg, var(--secondary), var(--primary));
            color: #fff; font-size: 16px; font-weight: 700; cursor: pointer;
            transition: all .3s ease; box-shadow: 0 6px 20px rgba(0,56,168,.35);
            display: flex; align-items: center; justify-content: center; gap: 8px;
        }
        .vgate-btn:hover:not(:disabled) { transform: translateY(-2px); box-shadow: 0 10px 28px rgba(0,56,168,.45); }
        .vgate-btn:disabled { opacity: .6; cursor: not-allowed; transform: none; }
        .vgate-hint { font-size: 11px; color: var(--muted); margin-top: 16px; }
...
<!-- ══════════ VOUCHER GATE ══════════ -->
<div class="vgate-backdrop" id="voucherGate">
    <div class="vgate-card">
        <div class="vgate-icon">🎟️</div>
        <h2>Enter Your Voucher Code</h2>
        <p>A valid voucher code is required to access the feedback form. Please ask the DICT staff for your code.</p>
        <div class="vgate-input-wrap">
            <input type="text" class="vgate-input" id="voucherInput"
                   placeholder="DICT-XXXXXX" maxlength="11" autocomplete="off"
                   oninput="formatVoucherInput(this)" onkeydown="if(event.key==='Enter') validateVoucher()">
        </div>
        <div class="vgate-msg" id="vgateMsgDisplay"></div>
        <button class="vgate-btn" id="vgateBtn" onclick="validateVoucher()">
            <span id="vgateBtnText">🔓 Validate Code</span>
        </button>
        <p class="vgate-hint">Voucher codes look like <strong>DICT-XXXXXX</strong></p>
    </div>
</div>
...
<input type="hidden" name="voucher_code" id="voucherCodeField">
    ...
    <div class="step" data-step="2">
        ...
        <div class="q-card" id="q_cc3">
            ...
            <div class="choice-group">
                <label class="choice-opt">
                    <input type="radio" name="cc3" value="2" id="cc3_no">
                    <div class="radio-dot"></div>
                    <span class="choice-text">No, I was not able to use the CC because…</span>
                </label>
                <label class="choice-opt">
                    <input type="radio" name="cc3" value="1" id="cc3_yes">
                    <div class="radio-dot"></div>
    ...
    <div class="step" data-step="3">
        ...
        <?php
        $sqds = [
            ['sqd1','SQD1','I spent an acceptable amount of time to complete my transaction','Responsiveness'],
            ['sqd2','SQD2','The office accurately informed and followed the transaction\'s requirements and steps','Reliability'],
            ['sqd3','SQD3','My online transaction (including steps and payment) was simple and convenient','Access & Facilities'],
            ['sqd4','SQD4','I easily found information about my transaction from the office or its website','Communication'],
            ['sqd5','SQD5','I paid an acceptable amount of fees for my transaction','Costs'],
            ['sqd6','SQD6','I am confident my online transaction was secure','Integrity'],
            ['sqd7','SQD7','The office\'s online support was available, or online support was quick to respond','Assurance'],
            ['sqd8','SQD8','I got what I needed from the government office','Outcome'],
        ];
...
(function initCC() {
    ...
    function checkCC2() {
        ...
        const show  = cc1v !== '3' && sel && sel.value !== '3';
...
function formatVoucherInput(el) {
    ...
    // Only auto-insert hyphen once user has typed the DICT prefix
    if (v.startsWith('DICT')) {
        v = v.slice(0, 4) + (v.length > 4 ? '-' + v.slice(4, 10) : '');
    }
    // Limit raw input to 10 chars (DICT + 6, hyphen is not counted since it's stripped above)
    if (!v.startsWith('DICT')) v = v.slice(0, 10);
    el.value = v;
    el.classList.remove('err', 'ok');
    document.getElementById('vgateMsgDisplay').textContent = '';
    document.getElementById('vgateMsgDisplay').className = 'vgate-msg';
}

function validateVoucher() {
    const input = document.getElementById('voucherInput');
    const btn   = document.getElementById('vgateBtn');
    const msg   = document.getElementById('vgateMsgDisplay');
    const code  = input.value.trim().toUpperCase();

    if (!code) {
        input.classList.add('err');
        msg.textContent = 'Please enter your voucher code.';
        msg.className = 'vgate-msg err';
        return;
    }

    btn.disabled = true;
    document.getElementById('vgateBtnText').textContent = '⏳ Validating…';

    const fd = new FormData();
    fd.append('code', code);

    fetch('../functions/validate_voucher.php', { method: 'POST', body: fd })
        .then(r => r.json())
        .then(data => {
            if (data.valid) {
                ...
                document.getElementById('voucherCodeField').value = code;
                setTimeout(() => {
                    document.getElementById('voucherGate').classList.add('hidden');
                    const header = document.getElementById('pageHeader');
                    const sc = document.getElementById('surveyContainer');
                    header.style.display = 'block';
                    sc.style.display = 'block';
                    // Force reflow so the browser registers the display change before animating
                    void sc.offsetWidth;
                    sc.style.animation = 'none';
                    void sc.offsetWidth;
                    sc.style.animation = 'slideUp .55s cubic-bezier(.22,1,.36,1)';
                }, 900);
            } else {
                input.classList.add('err');
                ...
                msg.className = 'vgate-msg err';
                btn.disabled = false;
                document.getElementById('vgateBtnText').textContent = '🔓 Validate Code';
            }
        })
        .catch(() => {
