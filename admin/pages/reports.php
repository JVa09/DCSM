...
if (isset($_GET['export']) && $_GET['export'] === 'json_data') {
    ...
    $sqd_labels = [1=>'Responsiveness',2=>'Reliability',3=>'Access and Facilities',4=>'Communication',5=>'Costs',6=>'Integrity',7=>'Assurance',8=>'Outcome',0=>'Overall'];

    $reports_to_generate = [['title'=>'OVERALL SUMMARY REPORT','tab_name'=>'Summary Report','filter'=>$filter_simple]];

    if (empty($f_service)) {
        $svc_q = $conn->query("SELECT id, service_name FROM services ORDER BY service_name ASC");
        if ($svc_q) {
    ...
    function gatherReportData($conn, $curr_filter) {
        ...
        $sqd = array_fill(0,9,[5=>0,4=>0,3=>0,2=>0,1=>0,'na'=>0]);
        $sq = $conn->query("SELECT sqd0,sqd1,sqd2,sqd3,sqd4,sqd5,sqd6,sqd7,sqd8 FROM csm_submissions WHERE $f");
        if($sq){while($row=$sq->fetch_assoc()){for($i=0;$i<=8;$i++){$v=(int)$row["sqd$i"];if($v>=1&&$v<=5)$sqd[$i][$v]++;else$sqd[$i]['na']++;}}}
        $data['sqd_counts']=$sqd;
        return $data;
    }
    ...
    foreach ($reports_to_generate as $rep) {
        ...
        if ($data['total_responses'] == 0 && $rep['title'] !== 'OVERALL SUMMARY REPORT') continue;
        ...
        if ($data['total_responses'] > 0) {
            ...
            $ws[]=['SERVICE QUALITY DIMENSIONS']; $ws[]=['Metric','Strongly Agree','Agree','Neither Agree','Disagree','Strongly Disagree','No Answer','Total','Overall'];
            foreach([1,2,3,4,5,6,7,8,0] as $i){ $c=$data['sqd_counts'][$i]; $t=array_sum($c); $v=$t-$c['na']; $sc=($v>0)?getPercent($c[5]+$c[4],$v):'0.00%'; $l=($i===0)?'Overall':$sqd_labels[$i]; $ws[]=[$l,$c[5],$c[4],$c[3],$c[2],$c[1],$c['na'],$t,$sc]; }
        }
...
$avg_rating       = $conn->query("SELECT AVG((sqd0+sqd1+sqd2+sqd3+sqd4+sqd5+sqd6+sqd7+sqd8)/9.0) as a FROM csm_submissions WHERE $filter_simple")->fetch_assoc()['a'] ?? 0;
...
$table_q = $conn->query("
    ...
    AVG((sqd0+sqd1+sqd2+sqd3+sqd4+sqd5+sqd6+sqd7+sqd8)/9.0) as avg_r,
    SUM(CASE WHEN ((sqd0+sqd1+sqd2+sqd3+sqd4+sqd5+sqd6+sqd7+sqd8)/9.0)>=4 THEN 1 ELSE 0 END) as sat_c
