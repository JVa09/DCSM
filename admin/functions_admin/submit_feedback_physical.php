...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // 1. Auto-generate Control Number (Your existing logic)
    $current_year = date('Y');
    ...
    // UPDATED: Added cc3_reason and one more ? placeholder
    ...
    // UPDATED: bind_param string updated to include 's' for cc3_reason
    // Count: 4s (start), 6i (CCs), 1s (reason), 9i (SQDs), 2s (end)
    $stmt->bind_param(
